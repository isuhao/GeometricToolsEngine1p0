...
// Insert a function call to glGetError after each OpenGL function call.
#define DO_GLERROR_CHECK
...
std::string::size_type const gMaxLine = 78;
    ...
    std::vector<std::string>& postamble)
{
    ...
    for (size_t j = 0; j < fnprotos.size(); ++j)
    {
        ...
        functions.push_back("    {");
        functions.push_back("        assert(false);");
        if (hasReturn)
        {
...
int main()
{
    ...
    std::ofstream output("GteOpenGL.cpp");
    assert(output);

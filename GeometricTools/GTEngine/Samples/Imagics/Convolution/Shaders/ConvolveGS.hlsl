...
void CSMain(int2 dt : SV_DispatchThreadID, int2 gt : SV_GroupThreadID)
{
    ...
    // are loaded.
    samples[gt.y + RADIUS][gt.x + RADIUS] = input[dt];
    if (gt.y >= RADIUS)
    {
        if (gt.y < NUM_Y_THREADS - RADIUS)
        {
            if (gt.x >= RADIUS)
            {
                ...
                {
                    // +0
                    samples[gt.y + RADIUS][gt.x + 2 * RADIUS] = input[dt + int2(+RADIUS, 0)];
                }
            }
            else
            {
                // -0
                samples[gt.y + RADIUS][gt.x] = input[dt + int2(-RADIUS, 0)];
            }
        }
        else
        {
            if (gt.x >= RADIUS)
            {
                ...
                {
                    // 0+
                    samples[gt.y + 2 * RADIUS][gt.x + RADIUS] = input[dt + int2(0, +RADIUS)];
                }
                else
                {
                    // ++
                    samples[gt.y + 2 * RADIUS][gt.x + 2 * RADIUS] = input[dt + int2(+RADIUS, +RADIUS)];
                    samples[gt.y + 2 * RADIUS][gt.x + RADIUS] = input[dt + int2(0, +RADIUS)];
                    samples[gt.y + RADIUS][gt.x + 2 * RADIUS] = input[dt + int2(+RADIUS, 0)];
                }
            }
            ...
            {
                // -+
                samples[gt.y + 2 * RADIUS][gt.x] = input[dt + int2(-RADIUS, +RADIUS)];
                samples[gt.y + 2 * RADIUS][gt.x + RADIUS] = input[dt + int2(0, +RADIUS)];
                samples[gt.y + RADIUS][gt.x] = input[dt + int2(-RADIUS, 0)];
            }
        }
    ...
    else
    {
        if (gt.x >= RADIUS)
        {
            ...
            {
                // 0-
                samples[gt.y][gt.x + RADIUS] = input[dt + int2(0, -RADIUS)];
            }
            else
            {
                // +-
                samples[gt.y][gt.x + 2 * RADIUS] = input[dt + int2(+RADIUS, -RADIUS)];
                samples[gt.y][gt.x + RADIUS] = input[dt + int2(0, -RADIUS)];
                samples[gt.y + RADIUS][gt.x + 2 * RADIUS] = input[dt + int2(+RADIUS, 0)];
            }
        }
        ...
        {
            // --
            samples[gt.y][gt.x] = input[dt + int2(-RADIUS, -RADIUS)];
            samples[gt.y][gt.x + RADIUS] = input[dt + int2(0, -RADIUS)];
            samples[gt.y + RADIUS][gt.x] = input[dt + int2(-RADIUS, 0)];
        }
    }

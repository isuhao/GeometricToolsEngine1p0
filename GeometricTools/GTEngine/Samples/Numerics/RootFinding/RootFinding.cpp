...
void FindRootsGPU(std::set<float>& roots)
{
    ...
    env.Insert(path + "/Samples/Numerics/RootFinding/Shaders/");
    ...
    HLSLDefiner definer;
    definer.SetString("FUNCTION_BODY", "(z - 1.1f)*(z + 2.2f)");
    std::shared_ptr<ComputeShader> cshader(ShaderFactory::CreateCompute(path,
        definer));
    if (!cshader)
    {
        LogError("Failed to compile RootFinder.hlsl.");
        return;
    }
    ...
    cshader->Set("rootBounds", acBuffer);

    engine.Execute(cshader, 512, 256, 1);

...
{
public:
    TestSubnormals(std::string const& hlslfile, std::string const& realname,
        Binary& result)
    {
        DX11Engine engine;

        std::shared_ptr<StructuredBuffer> inputBuffer(
            new StructuredBuffer(2, sizeof(Real)));
        ...
        std::shared_ptr<StructuredBuffer> outputBuffer(
            new StructuredBuffer(1, sizeof(Real)));
        ...
        HLSLDefiner definer;
        definer.SetString("REAL", realname);
        std::shared_ptr<ComputeShader> cshader(
            ShaderFactory::CreateCompute(hlslfile, definer));
        if (!cshader)
        {

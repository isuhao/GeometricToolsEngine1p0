...
StructuredBuffer<float3> vertices0;
StructuredBuffer<float3> vertices1;
...
void CSMain(uint2 dt : SV_DispatchThreadID)
{
    if (dt.x < numTriangles0 && dt.y < numTriangles1)
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
            V0[j] = mul(wMatrix0, float4(vertices0[3 * dt.x + j], 1.0f)).xyz;
            V1[j] = mul(wMatrix1, float4(vertices1[3 * dt.y + j], 1.0f)).xyz;
        }
#else
        [unroll]
        for (int j = 0; j < 3; ++j)
        {
            V0[j] = mul(float4(vertices0[3 * dt.x + j], 1.0f), wMatrix0).xyz;
            V1[j] = mul(float4(vertices1[3 * dt.y + j], 1.0f), wMatrix1).xyz;
        }
#endif

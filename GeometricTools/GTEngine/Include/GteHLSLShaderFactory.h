// Geometric Tools LLC, Redmond WA 98052
// Copyright (c) 1998-2014
// Distributed under the Boost Software License, Version 1.0.
// http://www.boost.org/LICENSE_1_0.txt
// http://www.geometrictools.com/License/Boost/LICENSE_1_0.txt
// File Version: 1.0.0 (2014/08/11)

#pragma once

#include "GTEngineDEF.h"
#include "GteHLSLDefiner.h"
#include "GteHLSLShader.h"

namespace gte
{

class GTE_IMPEXP HLSLShaderFactory
{
public:
    // Create a shader from an HLSL program in a file.
    static HLSLShader CreateFromFile(
        std::string const& filename,
        std::string const& entry,
        std::string const& target,
        HLSLDefiner const& definer,
        unsigned int compileFlags);

    // Create a shader from an HLSL represented as a string.
    static HLSLShader CreateFromString(
        std::string const& name,
        std::string const& source,
        std::string const& entry,
        std::string const& target,
        HLSLDefiner const& definer,
        unsigned int compileFlags);

private:
    // Common code for CreateFromFile and CreateFromString.
    static HLSLShader CompileAndReflect(
        std::string const& name,
        std::string const& source,
        std::string const& entry,
        std::string const& target,
        HLSLDefiner const& definer,
        unsigned int compileFlags);

    // Wrapper for the D3DCompile call.
    static ID3DBlob* CompileShader(
        std::string const& name,
        std::string const& source,
        std::string const& entry,
        std::string const& target,
        unsigned int compileFlags,
        D3D_SHADER_MACRO const* defines);

    // Support for shader reflection to obtain information about the HLSL
    // program.
    static bool ReflectShader(
        std::string const& name,
        std::string const& entry,
        std::string const& target,
        ID3DBlob* compiledCode,
        HLSLShader& shader);

    static bool GetDescription(ID3D11ShaderReflection* reflector,
        HLSLShader& shader);

    static bool GetInputs(ID3D11ShaderReflection* reflector,
        HLSLShader& shader);

    static bool GetOutputs(ID3D11ShaderReflection* reflector,
        HLSLShader& shader);

    static bool GetCBuffers(ID3D11ShaderReflection* reflector,
        HLSLShader& shader);

    static bool GetBoundResources(ID3D11ShaderReflection* reflector,
        HLSLShader& shader);

    static bool GetVariables(ID3D11ShaderReflectionConstantBuffer* cbuffer,
        unsigned int numVariables,
        std::vector<HLSLBaseBuffer::Member>& members);

    static bool GetTypes(ID3D11ShaderReflectionType* rtype,
        unsigned int numMembers, HLSLShaderType& stype);

    static bool IsTextureArray(D3D_SRV_DIMENSION dim);

    // Support for logging warnings and errors to a file.
    static void WriteLog(std::string const& message);
};

}

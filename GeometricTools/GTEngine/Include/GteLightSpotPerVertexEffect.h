...
class GTE_IMPEXP LightSpotPerVertexEffect : public VisualEffect
{
public:
    struct Lighting
    {
        ...
        Vector4<float> lightSpotCutoff;     // (angle,cosAngle,sinAngle,exp)
        Vector4<float> lightAttenuation;    // (*,*,*,w)
    };

    // Construction.
    LightSpotPerVertexEffect(Lighting const& lighting);
...
#include "GteLightSpotPerVertexEffect.inl"

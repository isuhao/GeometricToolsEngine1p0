    ...
    Ellipsoid3<Real> const& ellipsoid0, Ellipsoid3<Real> const& ellipsoid1)
{
    ...
    Matrix3x3<Real> R0(ellipsoid0.axis, true);
    Matrix3x3<Real> D0(
    ...
    Matrix3x3<Real> R1(ellipsoid1.axis, true);
    Matrix3x3<Real> D1(
    ...
    Real d1, Real d2, Real c0, Real c1, Real c2, int& numRoots, Real* roots)
{
    ...
    // infinite values.  If F(r) < 0, F(s) has two roots in the interval.
    // If F(r) = 0, F(s) has only one root in the interval.
    found = RootsBisection<Real>::Find(DF, invD1, invD2, -one, one,
        maxIterations, smid);
    LogAssert(found, "Unexpected condition.");
    if (F(smid) < zero)
    {
        // Pass in signs rather than infinities, because the bisector cares
        // only about the signs.
        found = RootsBisection<Real>::Find(F, invD1, smid, one, -one,
            maxIterations, s);
        LogAssert(found, "Unexpected condition.");
        roots[numRoots++] = s;
        found = RootsBisection<Real>::Find(F, smid, invD2, -one, one,
            maxIterations, s);
        LogAssert(found, "Unexpected condition.");
        roots[numRoots++] = s;
    }

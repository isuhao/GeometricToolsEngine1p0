...
// Each DX11Engine call using the immediate context is set to test that the
// thread in which the call occurs is the same one in which the device and
// immediate context were created.  This is a DX11 requirement.
#define GTE_USE_VERIFY_DEVICE_THREAD
...
public:
    // Thread information.  When creating a device for drawing, the following
    // objects must be created on the same thread:  window (HWND), message
    // pump, device, swap chain.  The immediate context obtained during device
    // creation must be used only on this thread.  The macro defined at the
    // end of this header, GTE_VERIFY_DEVICE_THREAD, occurs in DX11Engine
    // functions that use the immediate context.  If the thread is not the one
    // on which the device was created, an assertion is generated in debug
    // mode.  In release mode, the exception is logged to a file.
    inline DWORD GetDeviceThread() const;
    inline bool IsDeviceThread(DWORD threadID) const;
...
#if defined(GTE_USE_VERIFY_DEVICE_THREAD)
    #define GTE_VERIFY_DEVICE_THREAD\
        LogAssert(IsDeviceThread(GetCurrentThreadId()), "Invalid thread.")
#else
    #define GTE_VERIFY_DEVICE_THREAD
#endif

#include "GteDX11Engine.inl"

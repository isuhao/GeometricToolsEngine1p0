    ...
    Real bary[4], Real epsilon)
{
    ...
    Vector3<Real> e1ce2 = Cross(diff[1], diff[2]);
    Vector3<Real> e2ce0 = Cross(diff[2], diff[0]);
    Vector3<Real> e0ce1 = Cross(diff[0], diff[1]);
    if (std::abs(det) > epsilon)
    {
        Real invDet = ((Real)1)/det;
        bary[0] = Dot(diff[3], e1ce2)*invDet;
        bary[1] = Dot(diff[3], e2ce0)*invDet;
        bary[2] = Dot(diff[3], e0ce1)*invDet;

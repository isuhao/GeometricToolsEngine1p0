...
{
    return Delaunay2::operator()(numVertices, vertices, epsilon);
}
//----------------------------------------------------------------------------
template <typename InputType, typename ComputeType>
    ...
    std::array<int, 2> const& edge, std::vector<int>& outEdge)
{
    ...
    if (0 <= v0 && v0 < mNumVertices && 0 <= v1 && v1 < mNumVertices)
    {
    ...
    std::array<int, 2> const& edge, int v0Triangle, std::vector<int>& outEdge)
{
    ...
    GTE_CDT_REQUIRE(GetIndices(item->first, indices));
    int vNext = indices[(item->second + 1) % 3];
    int qr0 = mQuery.ToLine(v1, v0, vNext);
    while (item != link.end())
    {
        if (qr0 == 0)
        {
            ...
            Vector2<ComputeType> const& ctv0 = mComputeVertices[v0];
            Vector2<ComputeType> const& ctv1 = mComputeVertices[v1];
            Vector2<ComputeType> const& ctvnext = mComputeVertices[vNext];
        ...
        if (qr0 > 0)
        {
            ...
            qr0 = mQuery.ToLine(v1, v0, vNext);
            continue;
        }

        int vPrev = indices[(item->second + 2) % 3];
        int qr1 = mQuery.ToLine(v1, v0, vPrev);
        while (item != link.end())
        {
            if (qr1 == 0)
            {
                ...
                Vector2<ComputeType> const& ctv0 = mComputeVertices[v0];
                Vector2<ComputeType> const& ctv1 = mComputeVertices[v1];
                Vector2<ComputeType> const& ctvprev = mComputeVertices[vPrev];
            ...
            if (qr1 < 0)
            {
                // <v0,v1> is not in triangle.  Visit the next triangle.
                ++item;
                GetIndices(item->first, indices);
                vNext = vPrev;
                vPrev = indices[(item->second + 2) % 3];
                qr1 = mQuery.ToLine(v1, v0, vPrev);
                continue;
            }
    ...
    int v0, int v1, int vNext, int vPrev, std::vector<int>& outEdge)
{
    ...
    for (int i = 0; i < mNumTriangles; ++i)
    {
        ...
        int qr = mQuery.ToLine(vOpposite, v0, v1);
        if (qr == 0)
        {
    ...
    int v1, int v2) const
{
    ...
    Vector2<ComputeType> const& ctv0 = mComputeVertices[v0];
    Vector2<ComputeType> const& ctv1 = mComputeVertices[v1];
    Vector2<ComputeType> const& ctv2 = mComputeVertices[v2];
...
int ConstrainedDelaunay2<InputType, ComputeType>::GetLinkTriangle(int v) const
{
    ...
    for (int i = 0; i < mNumTriangles; ++i)
    {
        ...
        std::array<int, 3> indices;
        GTE_CDT_REQUIRE(GetIndices(tri, indices));
        for (int j = 0; j < 3; ++j)
        {
            if (v == indices[j])
            {
...
{
    std::array<int, 3> indices;
    GTE_CDT_REQUIRE_RET(GetIndices(tri, indices), -1);
    int indexOfV;
...
GetAdjInterior(int tri, int v0, int v1) const
{
    ...
    int adj = mAdjacencies[3 * tri + vIndex];
    if (adj >= 0)
    {
        for (int v2Index = 0; v2Index < 3; ++v2Index)
        {
            int v2 = mIndices[3 * adj + v2Index];
            if (v2 != v0 && v2 != v1)
            {
                return { adj, v2 };
            }
    ...
    int v0, int v1)
{
    if (tri >= 0)
    {
        ...
        if (adj >= 0)
        {
            ...
            mAdjacencies[3 * adj + v1Index] = tri;
        }

        mAdjacencies[3 * tri + v0Index] = adj;
    }
    ...
    int vTriangle, std::list<std::pair<int,int>>& link, bool& isOpen) const
{
    ...
    for (int i = 0; i < mNumTriangles; ++i)
    {
        ...
        else
        {
            ...
            {
                GetAdjacencies(tri, adjacencies);

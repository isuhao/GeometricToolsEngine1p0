    ...
    Sphere3<Real> const& sphere, Cone3<Real> const& cone)
{
    ...
    Vector3<Real> CmV = sphere.center - cone.vertex;
    Vector3<Real> D = CmV + (sphere.radius * invSin) * cone.axis;
    Real lenSqr = Dot(D, D);
    Real e = Dot(D, cone.axis);
    ...
    Sphere3<Real> const& sphere, Cone3<Real> const& cone)
{
    ...
    Vector3<Real> diff = sphere.center - cone.vertex;
    Real rSqr = sphere.radius * sphere.radius;
    ...
    if (lenSqr <= rSqr)
    {
        ...
        // intersect.
        result.intersect = true;
        result.point = cone.vertex;
        return result;
    }

    // Test whether the sphere center is inside the cone.
    Real dot = Dot(diff, cone.axis);
    Real dotSqr = dot*dot;
    Real cosSqr = cone.cosAngle * cone.cosAngle;
    ...
    if (discr >= (Real)0 && test >= (Real)0)
    {
        ...
        Vector3<Real> B = diff - dot * cone.axis;
        Real tmp = cone.sinAngle / uLen;
        result.point = t * (cone.cosAngle * cone.axis + tmp * B);
    }
    else
    {
        result.intersect = false;

    ...
    DX11Shader(shader)
{
    ...
    HRESULT hr = device->CreateVertexShader(&code[0], code.size(), linkage,
        &dxShader);
...
//----------------------------------------------------------------------------
DX11GraphicsObject* DX11VertexShader::Create(ID3D11Device* device,
    GraphicsObject const* object)
{
    if (object->GetType() == GT_VERTEX_SHADER)
    {
        return new DX11VertexShader(device,
...
void DX11VertexShader::Enable(ID3D11DeviceContext* context)
{
    if (mDXObject)
    {
        ...
        ID3D11VertexShader* dxShader =
            static_cast<ID3D11VertexShader*>(mDXObject);

...
//----------------------------------------------------------------------------
DX11TextureCubeArray::~DX11TextureCubeArray()
{
}
//----------------------------------------------------------------------------
DX11TextureCubeArray::DX11TextureCubeArray(ID3D11Device* device,
    ...
    DX11TextureArray(textureCubeArray)
{
    ...
    if (textureCubeArray->GetData())
    {
        unsigned int const numSubresources =
            textureCubeArray->GetNumSubresources();
...
//----------------------------------------------------------------------------
DX11GraphicsObject* DX11TextureCubeArray::Create(ID3D11Device* device,
    GraphicsObject const* object)
{
    if (object->GetType() == GT_TEXTURE_CUBE_ARRAY)
    {
        return new DX11TextureCubeArray(device,
...
//----------------------------------------------------------------------------
TextureCubeArray* DX11TextureCubeArray::GetTextureArray() const
{
    return static_cast<TextureCubeArray*>(mGTObject);
}
//----------------------------------------------------------------------------
ID3D11Texture2D* DX11TextureCubeArray::GetDXTextureArray() const
{
    return static_cast<ID3D11Texture2D*>(mDXObject);
}
//----------------------------------------------------------------------------
void DX11TextureCubeArray::CreateStaging(ID3D11Device* device,
...
//----------------------------------------------------------------------------
void DX11TextureCubeArray::CreateSRView(ID3D11Device* device,
    D3D11_TEXTURE2D_DESC const& tx)
{
    ...
    HRESULT hr = device->CreateShaderResourceView(GetDXTextureArray(), &desc,
        &mSRView);
    CHECK_HR_RETURN_NONE("Failed to create shader resource view");
}
//----------------------------------------------------------------------------
void DX11TextureCubeArray::CreateUAView(ID3D11Device* device,
    D3D11_TEXTURE2D_DESC const& tx)
{
    ...
    HRESULT hr = device->CreateUnorderedAccessView(GetDXTextureArray(), &desc,
        &mUAView);

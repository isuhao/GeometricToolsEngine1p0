...
//----------------------------------------------------------------------------
DX11GraphicsObject* DX11Texture3::Create(ID3D11Device* device,
    GraphicsObject const* object)
{
    if (object->GetType() == GT_TEXTURE3)
    {
        return new DX11Texture3(device, static_cast<Texture3 const*>(object));
    }
...
//----------------------------------------------------------------------------
Texture3* DX11Texture3::GetTexture() const
{
    return static_cast<Texture3*>(mGTObject);
}
//----------------------------------------------------------------------------
ID3D11Texture3D* DX11Texture3::GetDXTexture() const
{
    return static_cast<ID3D11Texture3D*>(mDXObject);
}
//----------------------------------------------------------------------------
void DX11Texture3::CreateStaging(ID3D11Device* device,
    D3D11_TEXTURE3D_DESC const& tx)
{
    D3D11_TEXTURE3D_DESC desc;
    desc.Width = tx.Width;
...
//----------------------------------------------------------------------------
void DX11Texture3::CreateSRView(ID3D11Device* device,
    D3D11_TEXTURE3D_DESC const& tx)
{
...
//----------------------------------------------------------------------------
void DX11Texture3::CreateUAView(ID3D11Device* device,
    D3D11_TEXTURE3D_DESC const& tx)
{

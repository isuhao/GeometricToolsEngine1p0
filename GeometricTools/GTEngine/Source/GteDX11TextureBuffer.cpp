...
//----------------------------------------------------------------------------
DX11TextureBuffer::DX11TextureBuffer(ID3D11Device* device,
    TextureBuffer const* tbuffer)
    :
    DX11Buffer(tbuffer),
    mSRView(nullptr)
{
...
//----------------------------------------------------------------------------
DX11GraphicsObject* DX11TextureBuffer::Create(ID3D11Device* device,
    GraphicsObject const* object)
{
    if (object->GetType() == GT_TEXTURE_BUFFER)
    {
        return new DX11TextureBuffer(device,
...
//----------------------------------------------------------------------------
TextureBuffer* DX11TextureBuffer::GetTextureBuffer() const
{
    return static_cast<TextureBuffer*>(mGTObject);
}
//----------------------------------------------------------------------------
ID3D11ShaderResourceView* DX11TextureBuffer::GetSRView() const
{
    return mSRView;
}
//----------------------------------------------------------------------------

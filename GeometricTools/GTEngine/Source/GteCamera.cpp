...
Camera::Camera(bool isPerspective)
    :
    mPosition(Vector4<float>::Zero()),
    mDVector(Vector4<float>::Zero()),
    mUVector(Vector4<float>::Zero()),
    mRVector(Vector4<float>::Zero()),
    ...
    mValidateCameraFrame(true)
{
    ...
    SetFrame(Vector4<float>::Origin(), -Vector4<float>::Basis2(),
        Vector4<float>::Basis1(), Vector4<float>::Basis0());
    ...
    Vector4<float> const& p01, float nearExtrude, float farExtrude)
{
    ...
    Matrix4x4<float> M(u0, u1, u2, q000, true);
    Matrix4x4<float> invM = Inverse(M);
    Vector4<float> a = invM*q111;
#else
    Matrix4x4<float> M(u0, u1, u2, q000, false);
    Matrix4x4<float> invM = Inverse(M);

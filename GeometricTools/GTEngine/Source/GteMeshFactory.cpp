    ...
    unsigned int numRadialSamples, float radius)
{
    ...
    for (unsigned int r0 = rsm1, r1 = 0, t = 0; r1 < numRadialSamples;
        r0 = r1++)
    {
...
//----------------------------------------------------------------------------
std::shared_ptr<Visual> MeshFactory::CreateBox(float xExtent, float yExtent,
    float zExtent)
{
    // Quantities derived from inputs.
    int numVertices = 8;
...
//----------------------------------------------------------------------------
std::shared_ptr<Visual> MeshFactory::CreateCylinderOpen(
    unsigned int numAxisSamples, unsigned int numRadialSamples, float radius,
    float height)
{
    // Quantities derived from inputs.
    ...
    for (unsigned int a = 0, i = 0; a < numAxisSamples; ++a)
    {
        ...
        for (unsigned int r = 0; r <= numRadialSamples; ++r, ++i)
        {
            ...
            nor = Vector3<float>(cs[r], sn[r], 0.0f);
            ...
            basis[0] = nor;
            ComputeOrthogonalComplement(1, basis);
            tcd = Vector2<float>(radialFraction, axisFraction);
    ...
    for (unsigned int a = 0, aStart = 0, t = 0; a < numAxisSamples - 1; ++a)
    {
        ...
        for (unsigned int i = 0; i < numRadialSamples;
            ++i, ++i0, ++i1, ++i2, ++i3)
        {
    ...
    if (visual)
    {
        ...
        visual->modelBound.SetCenter(Vector4<float>::Origin());
...
//----------------------------------------------------------------------------
std::shared_ptr<Visual> MeshFactory::CreateCylinderClosed(
    unsigned int numAxisSamples, unsigned int numRadialSamples, float radius,
    float height)
{
    // Create a sphere and then deform it into a closed cylinder.
    std::shared_ptr<Visual> visual = CreateSphere(numAxisSamples,
        numRadialSamples, radius);
    if (!visual)
    {
        return nullptr;
    }
    ...
    unsigned int numRadialSamples, float radius)
{
    ...
    for (unsigned int z = 1; z < zsm1; ++z)
    {
        ...
        {
            float radialFraction = r*invRS;  // in [0,1)
            Vector3<float> radial = Vector3<float>(cs[r], sn[r], 0.0f);
            ...
            basis[0] = nor;
            ComputeOrthogonalComplement(1, basis);
            tcd = Vector2<float>(radialFraction, 0.5f*(zFraction + 1.0f));
    ...
    pos = Vector3<float>(0.0f, 0.0f, -radius);
    if (mOutside)
    {
        nor = Vector3<float>(0.0f, 0.0f, -1.0f);
    }
    else
    {
        nor = Vector3<float>(0.0f, 0.0f, 1.0f);
    ...
    float outerRadius, float innerRadius)
{
    ...
    for (unsigned int c = 0; c < numCircleSamples; ++c)
    {
        ...
        for (unsigned int r = 0; r <= numRadialSamples; ++r, ++i)
        {
            ...
            nor = cosPhi*radial + sinPhi*Vector3<float>::Basis2();
            ...
            basis[0] = nor;
            ComputeOrthogonalComplement(1, basis);
            tcd = Vector2<float>(radialFraction, circleFraction);
...
std::shared_ptr<Visual> MeshFactory::CreateTetrahedron()
{
    ...
    Position(0) = Vector3<float>(0.0f, 0.0f, 1.0f);
    Position(1) = Vector3<float>(2.0f*sqrt2Div3, 0.0f, -oneThird);
    Position(2) = Vector3<float>(-sqrt2Div3, sqrt6Div3, -oneThird);
    Position(3) = Vector3<float>(-sqrt2Div3, -sqrt6Div3, -oneThird);
...
std::shared_ptr<Visual> MeshFactory::CreateHexahedron()
{
    ...
    Position(0) = Vector3<float>(-sqrtThird, -sqrtThird, -sqrtThird);
    Position(1) = Vector3<float>( sqrtThird, -sqrtThird, -sqrtThird);
    Position(2) = Vector3<float>( sqrtThird,  sqrtThird, -sqrtThird);
    Position(3) = Vector3<float>(-sqrtThird,  sqrtThird, -sqrtThird);
    Position(4) = Vector3<float>(-sqrtThird, -sqrtThird,  sqrtThird);
    Position(5) = Vector3<float>( sqrtThird, -sqrtThird,  sqrtThird);
    Position(6) = Vector3<float>( sqrtThird,  sqrtThird,  sqrtThird);
    Position(7) = Vector3<float>(-sqrtThird,  sqrtThird,  sqrtThird);
    for (unsigned int i = 0; i < numVertices; ++i)
    {
        pos = Position(i);
        nor = (mOutside ? pos : -pos);
...
std::shared_ptr<Visual> MeshFactory::CreateOctahedron()
{
    ...
    Position(0) = Vector3<float>( 1.0f, 0.0f, 0.0f);
    Position(1) = Vector3<float>(-1.0f, 0.0f, 0.0f);
    Position(2) = Vector3<float>( 0.0f, 1.0f, 0.0f);
    Position(3) = Vector3<float>( 0.0f,-1.0f, 0.0f);
    Position(4) = Vector3<float>( 0.0f, 0.0f, 1.0f);
    Position(5) = Vector3<float>( 0.0f, 0.0f,-1.0f);
    for (unsigned int i = 0; i < numVertices; ++i)
    {
        pos = Position(i);
        nor = (mOutside ? pos : -pos);
...
std::shared_ptr<Visual> MeshFactory::CreateDodecahedron()
{
    ...
    Position( 0) = Vector3<float>( a, a, a);
    Position( 1) = Vector3<float>( a, a,-a);
    Position( 2) = Vector3<float>( a,-a, a);
    Position( 3) = Vector3<float>( a,-a,-a);
    Position( 4) = Vector3<float>(-a, a, a);
    Position( 5) = Vector3<float>(-a, a,-a);
    Position( 6) = Vector3<float>(-a,-a, a);
    Position( 7) = Vector3<float>(-a,-a,-a);
    Position( 8) = Vector3<float>(  b,  c, 0.0f);
    Position( 9) = Vector3<float>( -b,  c, 0.0f);
    Position(10) = Vector3<float>(  b, -c, 0.0f);
    Position(11) = Vector3<float>( -b, -c, 0.0f);
    Position(12) = Vector3<float>(  c, 0.0f,  b);
    Position(13) = Vector3<float>(  c, 0.0f, -b);
    Position(14) = Vector3<float>( -c, 0.0f,  b);
    Position(15) = Vector3<float>( -c, 0.0f, -b);
    Position(16) = Vector3<float>(0.0f,   b,  c);
    Position(17) = Vector3<float>(0.0f,  -b,  c);
    Position(18) = Vector3<float>(0.0f,   b, -c);
    Position(19) = Vector3<float>(0.0f,  -b, -c);
    for (unsigned int i = 0; i < numVertices; ++i)
    {
        pos = Position(i);
        nor = (mOutside ? pos : -pos);
...
std::shared_ptr<Visual> MeshFactory::CreateIcosahedron()
{
    ...
    Position( 0) = Vector3<float>(  u,   v, 0.0f);
    Position( 1) = Vector3<float>( -u,   v, 0.0f);
    Position( 2) = Vector3<float>(  u,  -v, 0.0f);
    Position( 3) = Vector3<float>( -u,  -v, 0.0f);
    Position( 4) = Vector3<float>(  v, 0.0f,   u);
    Position( 5) = Vector3<float>(  v, 0.0f,  -u);
    Position( 6) = Vector3<float>( -v, 0.0f,   u);
    Position( 7) = Vector3<float>( -v, 0.0f,  -u);
    Position( 8) = Vector3<float>(0.0f,   u,   v);
    Position( 9) = Vector3<float>(0.0f,  -u,   v);
    Position(10) = Vector3<float>(0.0f,   u,  -v);
    Position(11) = Vector3<float>(0.0f,  -u,  -v);
    for (unsigned int i = 0; i < numVertices; ++i)
    {
        pos = Position(i);
        nor = (mOutside ? pos : -pos);
...
void MeshFactory::SetPlatonicTCoord(unsigned int i, Vector3<float> const& pos)
{
    ...
    {
        tcd[0] = 0.5f*(1.0f + atan2(pos[1], pos[0])*(float)GTE_C_INV_PI);
    }
    else
    {
        tcd[0] = 0.5f;

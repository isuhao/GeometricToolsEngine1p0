...
MarchingCubes::MarchingCubes ()
{
    ...
    memset(&mTable[0][0], 0, 256*41*sizeof(int));
...
//----------------------------------------------------------------------------
void MarchingCubes::Pack (int numV, int const* vpair, int numT,
    int const* itriple)
{
    // The item is already zeroed in the constructor.
    int* item = mTable[mEntry];
    item[0] = numV;
    item[1] = numT;

    // Store vertex pairs with minimum index occurring first.
    for (int i = 0, j = VSTART; i < numV; ++i, vpair += 2)
    {
        item[j++] = std::min(vpair[0], vpair[1]);
        item[j++] = std::max(vpair[0], vpair[1]);
    }

    // Store triangle triples as is.
    for (int i = 0, j = ISTART; i < 3*numT; ++i)
    {
        item[j++] = itriple[i];
    }
}
//----------------------------------------------------------------------------
void MarchingCubes::Unpack (int entry, int& numV, int vpair[2*MAX_VERTICES],
    int& numT, int itriple[3*MAX_TRIANGLES]) const
{
    int const* item = mTable[entry];
    numV = item[0];
    numT = item[1];

    memset(vpair, 0, 2*MAX_VERTICES*sizeof(int));
    memset(itriple, 0, 3*MAX_TRIANGLES*sizeof(int));
    memcpy(vpair, &item[VSTART], 2*numV*sizeof(int));
    memcpy(itriple, &item[ISTART], 3*numT*sizeof(int));
}
//----------------------------------------------------------------------------
void MarchingCubes::Bits0 (int[8])
{
    Pack(0, nullptr, 0, nullptr);
}
//----------------------------------------------------------------------------

...
HLSLShader HLSLShaderFactory::CreateFromFile(std::string const& filename,
    ...
    HLSLDefiner const& definer, unsigned int compileFlags)
{
    ...
    return CompileAndReflect(filename, source, entry, target, definer,
        compileFlags);
}
//----------------------------------------------------------------------------
HLSLShader HLSLShaderFactory::CreateFromString(std::string const& name,
    std::string const& source, std::string const& entry,
    std::string const& target, HLSLDefiner const& definer,
    unsigned int compileFlags)
{
    return CompileAndReflect(name, source, entry, target, definer,
        compileFlags);
}
//----------------------------------------------------------------------------
    ...
    unsigned int compileFlags)
{
    ...
    if (type == "vs_" || type == "gs_" || type == "ps_" || type == "cs_")
    {
        ID3DBlob* compiledCode = CompileShader(name, source, entry, target,
            compileFlags, definer.GetMacros());
        if (!compiledCode)
        {
    ...
    D3D_SHADER_MACRO const* defines)
{
    ...
    HRESULT hr = D3DCompile(source.c_str(), source.length(), name.c_str(),
        defines, include, entry.c_str(), target.c_str(), compileFlags, 0,
        &compiledCode, &errors);
    ...
    ID3DBlob* compiledCode, HLSLShader& shader)
{
    ...
    HRESULT hr = D3DReflect(buffer, numBytes, IID_ID3D11ShaderReflection,
        (void**)&reflector);
    if (FAILED(hr))
    {
...
//----------------------------------------------------------------------------
bool HLSLShaderFactory::GetDescription(ID3D11ShaderReflection* reflector,
    HLSLShader& shader)
{
...
//----------------------------------------------------------------------------
bool HLSLShaderFactory::GetInputs(ID3D11ShaderReflection* reflector,
    HLSLShader& shader)
{
...
//----------------------------------------------------------------------------
bool HLSLShaderFactory::GetOutputs(ID3D11ShaderReflection* reflector,
    HLSLShader& shader)
{
...
//----------------------------------------------------------------------------
bool HLSLShaderFactory::GetCBuffers(ID3D11ShaderReflection* reflector,
    HLSLShader& shader)
{
    ...
    for (UINT i = 0; i < numCBuffers; ++i)
    {
        ...
        if (cbDesc.Type == D3D_CT_CBUFFER)
        {
            ...
            {
                shader.Insert(HLSLConstantBuffer(resDesc, cbDesc.Size,
                    members));
            }
            else
            {
                for (UINT i = 0; i < resDesc.BindCount; ++i)
                {
                    shader.Insert(HLSLConstantBuffer(resDesc, i,
                        cbDesc.Size, members));
                }
            }
        }
        else if (cbDesc.Type == D3D_CT_TBUFFER)
        {
            ...
            if (resDesc.BindCount == 1)
            {
                shader.Insert(HLSLTextureBuffer(resDesc, cbDesc.Size,
                    members));
            }
            else
            {
                for (UINT i = 0; i < resDesc.BindCount; ++i)
                {
                    shader.Insert(HLSLTextureBuffer(resDesc, i,
                        cbDesc.Size, members));
                }
            }
        }
        else if (cbDesc.Type == D3D_CT_RESOURCE_BIND_INFO)
        {
            ...
            if (resDesc.BindCount == 1)
            {
                shader.Insert(HLSLResourceBindInfo(resDesc, cbDesc.Size,
                    members));
            }
            else
            {
                for (UINT i = 0; i < resDesc.BindCount; ++i)
                {
                    shader.Insert(HLSLResourceBindInfo(resDesc, i,
                        cbDesc.Size, members));
                }
            }
        }
...
//----------------------------------------------------------------------------
bool HLSLShaderFactory::GetBoundResources(ID3D11ShaderReflection* reflector,
    HLSLShader& shader)
{
    ...
    for (UINT i = 0; i < numResources; ++i)
    {
        ...
        if (FAILED(hr))
        {
            WriteLog("reflector->GetResourceBindingDesc error, hr = " +
                std::to_string(hr));
            return false;
        }
        ...
        ||  resDesc.Type == D3D_SIT_UAV_RWTYPED)   // RWTexture*
        {
            if (IsTextureArray(resDesc.Dimension))
            {
                ...
                else
                {
                    for (UINT i = 0; i < resDesc.BindCount; ++i)
                    {
                        shader.Insert(HLSLTextureArray(resDesc, i));
                    }
                }
            }
            else
            {
                ...
                else
                {
                    for (UINT i = 0; i < resDesc.BindCount; ++i)
                    {
                        shader.Insert(HLSLTexture(resDesc, i));
                    }
                }
            }
        }
        else if (resDesc.Type == D3D_SIT_SAMPLER)   // SamplerState
        {
            ...
            else
            {
                for (UINT i = 0; i < resDesc.BindCount; ++i)
                {
                    shader.Insert(HLSLSamplerState(resDesc, i));
                }
            }
        }
        else if (
        ...
        ||  resDesc.Type == D3D_SIT_UAV_RWBYTEADDRESS)  // RWByteAddressBuffer
        {
            ...
            else
            {
                for (UINT i = 0; i < resDesc.BindCount; ++i)
                {
                    shader.Insert(HLSLByteAddressBuffer(resDesc, i));
                }
            }
        }
        else
        {
            ...
            else
            {
                for (UINT i = 0; i < resDesc.BindCount; ++i)
                {
                    shader.Insert(HLSLStructuredBuffer(resDesc, i));
                }
            }
        }
    }
...
//----------------------------------------------------------------------------
bool HLSLShaderFactory::GetTypes(ID3D11ShaderReflectionType* rtype,

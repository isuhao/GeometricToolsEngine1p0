...
//----------------------------------------------------------------------------
Font::Font(unsigned int width, unsigned int height, char const* texels,
    ...
    mMaxMessageLength(maxMessageLength)
{
    ...
    mVertexBuffer.reset(new VertexBuffer(vformat, numVertices));
    ...
    mIndexBuffer.reset(new IndexBuffer(IP_TRIMESH, numTriangles,
        sizeof(unsigned int)));
    ...
    mTexture.reset(new Texture2(DF_R8_UNORM, width, height));
    ...
    mTextEffect.reset(new TextEffect(mTexture));
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
void Font::SetName(std::string const& name)
{
    mName = name;
    mVertexBuffer->SetName(mName + ":mVertexBuffer");
    mIndexBuffer->SetName(mName + ":mIndexBuffer");
    mTexture->SetName(mName + ":mTexture");
    mTextEffect->SetName(mName = ":mTextEffect");
}
//----------------------------------------------------------------------------

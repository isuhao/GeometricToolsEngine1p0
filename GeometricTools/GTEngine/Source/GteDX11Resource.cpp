...
//----------------------------------------------------------------------------
Resource* DX11Resource::GetResource() const
{
    return static_cast<Resource*>(mGTObject);
}
//----------------------------------------------------------------------------
ID3D11Resource* DX11Resource::GetDXResource() const
{
    return static_cast<ID3D11Resource*>(mDXObject);
}
//----------------------------------------------------------------------------
ID3D11Resource* DX11Resource::GetStagingResource() const
{
    return mStaging;
}
//----------------------------------------------------------------------------
D3D11_MAPPED_SUBRESOURCE DX11Resource::MapForWrite(
    ID3D11DeviceContext* context, unsigned int sri)
{
    D3D11_MAPPED_SUBRESOURCE mapped;
    HRESULT hr = context->Map(GetDXResource(), sri, D3D11_MAP_WRITE_DISCARD,
        0, &mapped);
    if (FAILED(hr))
    {
...
bool DX11Resource::PreparedForCopy(D3D11_CPU_ACCESS_FLAG access) const
{
    ...
    if (!mDXObject)
    {
        LogWarning("Resource does not exist.");
        return false;
    }
    if (!mStaging)
    {
        LogWarning("Staging resource does not exist.");
        return false;
    }
    ...
    {
        LogWarning("Staging resource has incorrect CPU access.");
        return false;
    }

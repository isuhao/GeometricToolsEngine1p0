...
//----------------------------------------------------------------------------
DX11GeometryShader::DX11GeometryShader(ID3D11Device* device,
    Shader const* shader)
    :
    DX11Shader(shader)
{
    ...
    HRESULT hr = device->CreateGeometryShader(&code[0], code.size(), linkage,
        &dxShader);
...
//----------------------------------------------------------------------------
DX11GraphicsObject* DX11GeometryShader::Create(ID3D11Device* device,
    GraphicsObject const* object)
{
    if (object->GetType() == GT_GEOMETRY_SHADER)
    {
        return new DX11GeometryShader(device,
...
void DX11GeometryShader::Enable(ID3D11DeviceContext* context)
{
    if (mDXObject)
    {
        ...
        UINT numInstances = 0;
        ID3D11GeometryShader* dxShader =
            static_cast<ID3D11GeometryShader*>(mDXObject);

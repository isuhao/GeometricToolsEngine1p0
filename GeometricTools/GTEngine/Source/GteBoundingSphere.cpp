...
//----------------------------------------------------------------------------
void BoundingSphere::TransformBy (Transform<float> const& transform,
...
#endif
    sphere.SetRadius(L1Norm(transform)*GetRadius());
}
//----------------------------------------------------------------------------
    ...
    Vector4<float> const& direction, float tmin, float tmax) const
{
    ...
    Vector4<float> segOrigin = origin + segExtent*direction;
    ...
    return qm > 0.0f && std::abs(a1) < segExtent;
}
//----------------------------------------------------------------------------

...
//----------------------------------------------------------------------------
DX11GraphicsObject* DX11RawBuffer::Create(ID3D11Device* device,
    GraphicsObject const* object)
{
    if (object->GetType() == GT_RAW_BUFFER)
    {
        return new DX11RawBuffer(device,
...
//----------------------------------------------------------------------------
RawBuffer* DX11RawBuffer::GetRawBuffer() const
{
    return static_cast<RawBuffer*>(mGTObject);
}
//----------------------------------------------------------------------------
ID3D11ShaderResourceView* DX11RawBuffer::GetSRView() const
{
    return mSRView;
}
//----------------------------------------------------------------------------
ID3D11UnorderedAccessView* DX11RawBuffer::GetUAView() const
{
    return mUAView;
}
//----------------------------------------------------------------------------

...
bool DX11Engine::CreateSwapChain(HWND handle, UINT xSize, UINT ySize)
{
    ...
    hr = dxgiFactory->CreateSwapChain(dxgiDevice, &desc, &mSwapChain);
    CHECK_HR_RETURN("Failed to create swap chain", false);

#ifdef GTE_USE_NAMED_DX11_OBJECTS
    hr = SetPrivateName(mSwapChain, "DX11Engine::mSwapChain");
    CHECK_HR_RETURN("Failed to set private name", false);
#endif
...
bool DX11Engine::CreateBackBuffer(UINT xSize, UINT ySize)
{
...
#ifdef GTE_USE_NAMED_DX11_OBJECTS
    hr = SetPrivateName(mColorBuffer, "DX11Engine::mColorBuffer");
    CHECK_HR_RETURN("Failed to set private name", false);
#endif
...
#ifdef GTE_USE_NAMED_DX11_OBJECTS
    hr = SetPrivateName(mColorView, "DX11Engine::mColorView");
    CHECK_HR_RETURN("Failed to set private name", false);
#endif
    ...
    hr = mDevice->CreateTexture2D(&desc, nullptr, &mDepthStencilBuffer);
    CHECK_HR_RETURN("Failed to create depth-stencil buffer", false);

#ifdef GTE_USE_NAMED_DX11_OBJECTS
    hr = SetPrivateName(mDepthStencilBuffer,
...
#ifdef GTE_USE_NAMED_DX11_OBJECTS
    hr = SetPrivateName(mDepthStencilView, "DX11Engine::mDepthStencilView");
    CHECK_HR_RETURN("Failed to set private name", false);
#endif
...
{
    mDefaultFont.reset(new FontArialW400H18(256));
    mDefaultBlendState.reset(new BlendState());
    mDefaultDepthStencilState.reset(new DepthStencilState());
    mDefaultRasterizerState.reset(new RasterizerState());

#ifdef GTE_USE_NAMED_DX11_OBJECTS
    mDefaultBlendState->SetName("CEngine::mDefaultBlendState");
    mDefaultDepthStencilState->SetName("CEngine::mDefaultDepthStencilState");
    mDefaultRasterizerState->SetName("CEngine::mDefaultRasterizerState");
#endif
...
void DX11Engine::DestroyDefaultObjects()
{
    ...
    if (mDefaultBlendState)
    {
        ...
        mActiveBlendState.reset();
    }

    if (mDefaultDepthStencilState)
    {
        Unbind(mDefaultDepthStencilState);
        mDefaultDepthStencilState.reset();
        mActiveDepthStencilState.reset();
    }
    ...
    std::shared_ptr<Texture2> const& texture, DX11Engine* engine)
{
    ...
    if (dxTexture)
    {
        DX11GraphicsObject* dxShared = new DX11Texture2(mDevice,
